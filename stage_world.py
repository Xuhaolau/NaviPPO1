...
class StageWorld():
    def __init__(self, beam_num, index, num_env):
        ...
        # Odometry: robot's position   ------- the later one is the recall function
        ...
        # including: speed state speed_GT state_GT
        self.speed = None
        self.state = None
        self.speed_GT = None
        self.state_GT = None
    ...
    #-----------------------------------------------recall function--------------------------------------
    #   object_state_topic recall function
    #   get ground_truth of : state(x,y,)   speed(v, angular)
    ...
    #   odometry recall function
    #   get state(x,y,)  speed(Vx,angular)
    ...
    # cess the raw laser scan into 512 dimension
    ...
    def generate_random_goal(self):
        ...
        while (dis_origin > 6 or dis_goal > 6 or dis_goal < 5) and not rospy.is_shutdown():
            x = np.random.uniform(-5, 5)
            y = np.random.uniform(-5, 5)
            dis_origin = np.sqrt(x ** 2 + y ** 2)
            dis_goal = np.sqrt((x - self.init_pose[0]) ** 2 + (y - self.init_pose[1]) ** 2)
    ...
    def get_reward_and_terminate(self, t):
        ...
        reward_a = (self.pre_distance - self.distance) * 2
        reward_c = 0
        reward_s = 0
        # reward_w = 0
        result = 0
        ...
        if t > 128:
            terminate = True
            result = 'Time out'
        reward = reward_a + reward_c + reward_s
